...
-- Check if any record's 'prd_start_dt' begin after the 'prd_end_dt'
-- Expectation: No Results
SELECT * 
FROM silver.crm_prd_info
...
-- Check for Invalid dates (e.g. Negative, More than 8 chars, Out of bounds in future or past)
...
-- Check if any record's 'sls_order_dt' begin after the 'sls_ship_dt' or 'sls_due_dt'
-- Expectation: No Results
SELECT * 
...
-- Check Data Consistency: Sales = Quantity * Price, and check for NULL and negative values
-- Expectation: No Results
SELECT DISTINCT 
	sls_sales,
    sls_quantity,
